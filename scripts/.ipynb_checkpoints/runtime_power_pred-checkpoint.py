...
def prepare_multivariate_sequences(metrics_df, dcgm_input_features,
    ...
    for _, row in metrics_df.iterrows():
        ...
        # Reshape cause there are 4 GPUs on a node
        # At each timestep, we have 4 different metrics coming from 4 different GPUs
        if n_timesteps_raw % n_gpus != 0:
            n_timesteps_raw = (n_timesteps_raw // n_gpus) * n_gpus 
        
        n_timesteps = n_timesteps_raw // n_gpus  # Actual number of timesteps
        usable = n_timesteps - window
        
        if usable <= 0:
            continue
        
        # Reshape each array (n_timesteps_raw,) to (n_timesteps, n_gpus)
        # Then average across GPUs (n_timesteps, n_gpus) to (n_timesteps,)
        arrs_reshaped = []
        for a in arrs:
            a_trimmed = a[:n_timesteps_raw]
            a_2d = a_trimmed.reshape(n_timesteps, n_gpus)  # (timesteps, gpus)
            a_avg = a_2d.mean(axis=1)  # (timesteps,) - average across GPUs
            arrs_reshaped.append(a_avg)
        
        # Same for the target
        target_trimmed = target_arr[:n_timesteps_raw]
        target_2d = target_trimmed.reshape(n_timesteps, n_gpus)
        target_avg = target_2d.mean(axis=1)  # (timesteps,)
        
        stacked = np.vstack(arrs_reshaped).T  # shape (n_timesteps, n_features_total)
        ...
        for i in range(usable):
            ...
            y_list.append(target_avg[i+window])  # average target is predicted
            
    X = np.stack(X_list)   # (samples, window * n_features + static)
...
=======
>>>>>>> 4e174ba (repo update with new experiments)
def bin_power(y):
    return pd.cut(
        y.ravel(),
